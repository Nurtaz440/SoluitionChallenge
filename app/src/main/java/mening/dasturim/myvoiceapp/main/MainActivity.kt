...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityMainBinding.playBtn.setOnClickListener {
            if (!run) {
                activityMainBinding.playBtn.setImageResource(R.drawable.ic_baseline_pause_24)
                speak()
                run = true
            } else {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                val bitmapDrawable: BitmapDrawable =
                    activityMainBinding.imageTv.drawable as BitmapDrawable
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                Toast.makeText(this, "The Language specified is not supported!", Toast.LENGTH_SHORT)
                    .show()
            } else {
                activityMainBinding.playBtn.isEnabled = true
